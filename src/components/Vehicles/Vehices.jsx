...
const Vehicles = () => {
    ...
    // Fetch all vehicles initially or when category changes
    useEffect(() => {
        const fetchVehicles = async () => {
            ...
            try {
                ...
                setFilteredVehicles(allVehicles); // initially show all
                setShowAll(false);
            } catch (error) {
    ...
    const handleSearch = ({ location, category: searchCategory, min, max }) => {
        ...
        setShowAll(false); // reset toggle when new search is applied
    };

    // Slice displayed vehicles
        ...
        <Container>
            ...
            {Array.isArray(filteredVehicles) && filteredVehicles.length > 0 ? (
                ...
                <div className="flex items-center justify-center min-h-[calc(100vh-300px)]">
                    <h2 className="text-xl font-semibold text-gray-700">No vehicles found</h2>
                </div>
            )}
        </Container>
    );
