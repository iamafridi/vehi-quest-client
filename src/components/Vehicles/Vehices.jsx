...
const Vehicles = () => {
        ...
        return [...vehiclesArray].sort((a, b) => {
            switch (sortType) {
                ...
                case 'availability':
                    ...
                    const aSoldOut = a.soldOut || a.status === 'sold_out' || false;
                    const bSoldOut = b.soldOut || b.status === 'sold_out' || false;
    ...
    const stats = useMemo(() => {
        ...
        return filteredVehicles.reduce((acc, vehicle) => {
            acc.total++;
            const isSoldOut = vehicle.soldOut || vehicle.status === 'sold_out';
            if (isSoldOut) {
                acc.soldOut++;
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8 w-full">
                        {displayedVehicles.map((vehicle, index) => (
                            <Card
                                ...
                                vehicle={vehicle}
                                priority={index < 4}
                                isSoldOut={vehicle.soldOut || vehicle.status === 'sold_out'}
                            />
                        ))}
                    </div>
